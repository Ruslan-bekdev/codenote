...
fatal: unable to auto-detect email address (got 'toopi@DESKTOP-A29EANT.(none)')!!!!РЕШЕНИЕ
...
Проверка текущей ветви {{git branch}}Если текущая ветвь master {{git push origin master}}Если нужна именно ветвь main то{{git checkout -b main}}Ветвь создана. Теперь пушим{{git push -u origin main}}`},{summary:`Связь локальных и удаленных ветвей (
...
upstream, see 'push.autoSetupRemote' in 'git help config'.!!!!РЕШЕНИЕ
Локальная ветка main не связана с удалённой веткой на GitHub.
Нужно установить upstream (верхний уровень){{git push --set-upstream origin main}}Пояснение:
...
в консоли выходит ошибка)`,content:`!!!!import declarations may only appear at top level of a module!!!!РЕШЕНИЕ
...
PS C:\\Users\\Zooppio\\WebstormProjects\\codenote>!!!!РЕШЕНИЕ
Удаляем старые модули и кэши{{rm -rf node_modules}} (если есть){{rm package-lock.json #}}{{npm cache clean --force}}Устанавливаем заново{{npm install}}Принудительно переустановить esbuild
Иногда esbuild нужно пересобрать для правильной архитектуры{{npm rebuild esbuild}}`},{summary:"Ошибка при деплое vite проекта 3",content:`!!!!Fatal process out of memory: Zone Allocation failed
<--- Last few GCs --->
...
23: 00007FF802F62651!!!!РЕШЕНИЕ
Ошибка указывает на то, что данных слишком много. Нужно просто очистить кэш 
{{npm cache clean --force}}`},{summary:"Если git push не работает (Не изменяет ничего в гитхабе, но ошибок нет)",content:`РЕШЕНИЕ
...
~~РЕШЕНИЕ
Создать текстовый файл и написать туда код. Переименовать файл с .txt на .regи запустить от имени администратора (Хотя у меня получилось и обычным запуском). Вот код: {{Windows Registry Editor Version 5.00
