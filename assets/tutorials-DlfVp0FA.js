...
Чтобы переименовать текущую ветвь в мейн: {{git branch -M main}}При первом пуше: {{git push -u origin main}}`},{summary:"Создание React проекта и первичная настройка",content:`npx create-react-app . --template typescript (в конце это если прорект будет тайпскрипт)
...
logo.svg`},{summary:"Создание Vite проекта и первичная настройка",content:`VITE{{npm create vite@latest .}}Команда запуска: {{npm run dev}}Далее добавляем плагин
...
2.Настройка проекта
Пекейдж Джейсон:
Добавить homepage и указать ему скопированную ссылку{{npm i gh-pages}}В scripts прописать следующие ключи
{{"deploy": "gh-pages -d build",
"predeploy": "npm run build"}}Закомитить все и запушить

3.Деплой{{npm run deploy}}Настройка-pages:
...
2.Настройка проекта{{npm i gh-pages}}В scripts прописать следующие ключи после ключа "dev"
{{"deploy": "gh-pages -d dist",
"predeploy": "npm run build"}}указать в файле vite.config.js ключ base с ссылкой в значении (если такого файла нет - создать)
Например так{{base: 'https://ruslan-bekdev.github.io/codenote/',}}В итоге должно быть так {{import { defineConfig } from 'vite';
...
выполняем {{npm i bcryptjs}}выполняем команду node и втсавляем туда это:
...
Переходим на сайт по ссылке{{https://www.amd.com/en/support/kb/release-notes/rn-rad-win-15-7-1}}Скачиваем по подходящей ссылке
...
Питание, "PowerPlay", убрать галочку с "Включить Vari-Bright"`},{summary:"Установка Microsoft Store если его нет (например после моего тутора на винду)",content:"{{wsreset –i}} в командную строку"},{summary:"Как починить рассинхрон аудио и видео через ffmpeg",content:`Переходим по ссылке {{https://ffmpeg.org/download.html#build-windows}}В моём случае переходим к Виндовс и выбираем один из двух Билдов. Лично я выбрал билд от gyan.dev
...
Команда выглядит так "{{ffmpeg -i "" -itsoffset -1 -i "" -map 0:v -map 1:a -c copy ""}}Также можно исправлять сразу несколько видео в одной ппаке, то подходит следующий код{{for %f in ("СЮДА") do ffmpeg -i "%f" -itsoffset -1 -i "%f" -map 0:v -map 1:a -c copy "ТУДА"}}В этом коде СЮДА надо написать путь к папке с видео как обычно, но вместо названия написать *, а ТУДА написать путь к файлу сохранения но вместо название написать %~nf, таким образом он сохранится под старым названием
...
//# sourceMappingURL=tutorials-DlfVp0FA.js.map
